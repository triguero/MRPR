 ...
 * y convertirlos a formatoorg.apache.mahout.keel.
 *
...
//Variable auxiliar para almacenar el texto de todos los descendientes de un nodo.
     ...
     * html indicado mediante la variable pathnameInput a formatoorg.apache.mahout.keel.en el fichero
     * indicado por la ruta pathnameOutput.
     *
     * @param pathnameInput ruta del fichero con formato html donde se almacena la tabla
     * @param pathnameOutput ruta con los datos en formatoorg.apache.mahout.keel.
     *
     * @throws Exception */
...
//Reservamos memoria para almacenar la definición de los atributos y de los datos
            ...
            attribute = neworg.apache.mahout.keel.Dataset.Attribute[numAttributes];
            data = new Vector[numAttributes];
            types = new Vector[numAttributes];

            for (i = 0; i < numAttributes; i++) {
                attribute[i] = neworg.apache.mahout.keel.Dataset.Attribute();
                data[i] = new Vector();
                types[i] = new Vector();
            }
