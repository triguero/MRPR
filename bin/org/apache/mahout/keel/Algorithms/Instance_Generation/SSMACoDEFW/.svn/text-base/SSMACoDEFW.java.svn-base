...
package org.apache.mahout.keel.Algorithms.Instance_Generation.SSMACoDEFW;
...
public class SSMACoDEFW extends Metodo {
  ...
  public SSMACoDEFW (String ficheroScript) {
	super (ficheroScript);
    
  }

  public SSMACoDEFW (String ficheroScript, InstanceSet train) {
...
public Prototype mutant(Prototype population[], int actual, int mejor, double SFi){
  ...
  /**
   * It returns a population optimized with SFLSDE and the fitness value.
   * @param population
   * @return
   */
  
  public Pair<PrototypeSet[], double[]> SFLSDE(PrototypeSet [] population, Prototype weights)
  {
      ...
      for(int i=0; i< PopulationSize; i++){
		  ...
		  fitness[i] = classficationAccuracy1NN_weights(nominalPopulation, weights,trainingDataSet);   // PSOfitness
	 ...
	 double bestFitness=fitness[0];
	  ...
	  for(int i=1; i< PopulationSize;i++){
	   ...
	   for(int iter=0; iter< MaxIter; iter++){ // Main loop
		   ...
		   for(int i=0; i<PopulationSize; i++){
			   ...
			   nominalPopulation = new PrototypeSet();
		       nominalPopulation.formatear(population[i]);
		       fitness[i] = classficationAccuracy1NN_weights(nominalPopulation, weights,trainingDataSet);
		       
		       nominalPopulation = new PrototypeSet();
		       nominalPopulation.formatear(crossover[i]);
		       
			   double trialVector = classficationAccuracy1NN_weights(nominalPopulation, weights,trainingDataSet);
			  ...
			  if(fitness[i]>bestFitness){
				  bestFitness = fitness[i];
				  bestFitnessIndex=i;
				  System.out.println("FITNESSsflsde=" + bestFitness);
			  }
	  ...
	  //  nominalPopulation = new PrototypeSet();
       ...
       // System.err.println("\n% de acierto en training Nominal CON PESOS " + classficationAccuracy1NN_weights(nominalPopulation, weights,trainingDataSet) );
			  
		//  nominalPopulation.print();

  
		Pair<PrototypeSet[], double[]> salida = new Pair<PrototypeSet[], double[]>(population,fitness);
...
/** Main method */
  ...
  public PrototypeSet reduceSet(PrototypeSet initial){
	 ...
	 // population[0].print();
	  ...
	  double Weights[] = new double[population[0].get(0).numberOfInputs()]; // weights vector for the FEATURE SPACE!
      ...
      for(int i=1; i< PopulationSize; i++){
		  ...
		  fitness[i] = classficationAccuracy1NN(nominalPopulation,trainingDataSet);   // PSOfitness
		  fitness_bestPopulation[i] = fitness[i]; // Initially the same fitness.
	  }
	 ...
	 double bestFitness=fitness[0];
	  ...
	  for(int i=1; i< PopulationSize;i++){
	   ...
	   Weights= FeatureWeighting(population[bestFitnessIndex],true, Weights).clone(); // con el máximo de iteraciones.
	  ...
	  int iter =0;
		...
		while(iter<MAX_ITER){
			   ...
			   Pair <PrototypeSet[], double[]> de = SFLSDE(population, Pesos); // debe retornar los fitness tb.
			   population = de.first().clone();
			   fitness =de.second().clone();
			 ...
			 //  nominalPopulation = new PrototypeSet();
		      // nominalPopulation.formatear(population[bestFitnessIndex]);
		       // System.err.println("\n% de acierto en training Nominal CON PESOS " + classficationAccuracy1NN_weights(nominalPopulation, Pesos,trainingDataSet) );	  
				...
				Weights= FeatureWeighting(population[bestFitnessIndex],false, Weights).clone();  // con menos iteraciones.
...
/*      System.out.println(ev + " - (" + umbralOpt + ")" +
 ...
 //   training.print(); // Conjunto devuelto POR SSMA
       ...
       // KeelFile.copy(inputFilesPath.get(TEST), outputFilesPath.get(TEST));
	    ...
	    for(int m=0; m< this.pesos.length; m++){
	    	System.out.print(pesos[m]+", ");
	    }
	    
	    System.out.println("PESOSend");
	    
	    if(!this.Script.equals("NOFILE")){ // Solo clasificar si hay fichero de config..
	       ...
	       //Working on training
	        ...
	        for (i=0; i<trainingDataSet.size(); i++) {
	             ...
	             trainPrediction[i][0] = evaluateWeighting(trainingDataSet.get(i).getInputs(),DE.prototypeSetTodouble(), nClases, DE.getClases(), 1, pesos);
	        ...
	        for (i=0; i<realClass.length; i++) {
		      ...
		      prediction[i][0]= evaluateWeighting(testDataSet.get(i).getInputs(),DE.prototypeSetTodouble(), nClases, DE.getClases(), 1, pesos);
	        }
  ...
  public void leerConfiguracion (String ficheroScript) {
    ...
    if(ficheroScript.equals("NOFILE")){
	    ...
	    this.tau[3] =  0.07;
	    this.Strategy = 3;
	    this.iterFWinitial = 1000;   
	    this.iterFW = 200;   
	    this.PopulationFW = 25;   
	    ...
	    tokens = new StringTokenizer (linea, "=");
	    tokens.nextToken();
	    this.iterFWinitial = Integer.parseInt(tokens.nextToken().substring(1));   
	
	    linea = lineasFichero.nextToken();
	    tokens = new StringTokenizer (linea, "=");
	    tokens.nextToken();
	    this.iterFW = Integer.parseInt(tokens.nextToken().substring(1));   
	    
	    
	    linea = lineasFichero.nextToken();
	    tokens = new StringTokenizer (linea, "=");
	    tokens.nextToken();
	    this.PopulationFW = Integer.parseInt(tokens.nextToken().substring(1));   
    }
    
    System.out.print("\nIsaac dice: Population FW ="+this.PopulationFW +"\n");
