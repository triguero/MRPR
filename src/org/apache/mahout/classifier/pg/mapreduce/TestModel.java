...
public class TestModel extends Configured implements Tool{
  ...
  // TODO Auto-generated method stub
    ...
    Option save = obuilder.withLongName("save Reduce set as plain text").withShortName("save").withRequired(false).withArgument(
	        abuilder.withName("path").withMinimum(1).withMaximum(1).create()).
	        withDescription("Preprocessed set path").create();
	...
	Group group = gbuilder.withName("Options").withOption(inputOpt).withOption(testOpt).withOption(header).withOption(preprocessedOpt).withOption(save)
	...
	try {
      ...
      String saveName = cmdLine.getValue(save).toString();
      
      
      if(saveName.equalsIgnoreCase("true")){
    	  onlySave=true;
      }
  ...
  private void testPreprocessedSet() throws IOException, ClassNotFoundException, InterruptedException {
   ...
   // PrototypeSet RS=PGUtils.readPrototypeSet(preprocessedPath.toString());
    ...
    if(onlySave){
    	System.out.println("Guardo el dataset como ReducidoPG.data en ~");
        PrototypeSet RS = PrototypeSet.load(getConf(), preprocessedPath, cabecera.getHeader());

        System.out.println(RS.size());
        
		outFS = outputPath.getFileSystem(getConf());
		FSDataOutputStream ofile = null;
		Path filenamePath = new Path(outputPath, "ReducidoPG").suffix(".data");
