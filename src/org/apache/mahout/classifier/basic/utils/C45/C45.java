...
package org.apache.mahout.classifier.basic.utils.C45;
...
import org.apache.mahout.keel.Algorithms.Decision_Trees.C45.Algorithm;
...
public class C45 extends Algorithm {
   ...
   * @throws Exception	If the algorithm cannot be executed.
   */
  public C45(InstanceSet trainfn, Context context) throws Exception {
      try {
    ...
    public ArrayList<Integer> getPredicciones(){
    	ArrayList<Integer> classesp =new ArrayList<Integer>(); // = new int[testDataset.numItemsets()];
    	
   	   context.progress();

    	 for(int i = 0 ; i < testDataset.numItemsets() ; ++i)
			try {
				classesp.add((int) evaluateItemset(testDataset.itemset(i)));
			} catch (Exception e) {
				e.printStackTrace();
			}
    ...
    public ArrayList<Integer> getPredicciones(FileSystem fs, Path test) throws IOException{
    	...
    	FSDataInputStream input = fs.open(test);
		    ...
		    while (scanner.hasNextLine()) {
		        ...
		        try {
					testHadoop.readSet(instancias, true) ;
				} catch (DatasetException | HeaderFormatException e1) {
					// TODO Bloque catch generado automáticamente
					e1.printStackTrace();
				}  // Tienes que hacer una nueva readSET en INstanceSEt.java
		        
		        Dataset isC45=new Dataset(testHadoop);
		        
		        
		    	 for(int i = 0 ; i < isC45.numItemsets() ; ++i)
		 			try {
		 				classesp.add((int) evaluateItemset(isC45.itemset(i)));
		 			} catch (Exception e) {
		 				e.printStackTrace();
		 			}
