...
public class KeelHeader extends Configured implements Tool{
  ...
  // TODO Auto-generated method stub
	...
	Group group = gbuilder.withName("Options").withOption(inputOpt).withOption(testOpt).withOption(header).withOption(outputOpt).withOption(helpOpt).create(); 
  ...
  private void setRanges() throws IOException, ClassNotFoundException, InterruptedException {
   ...
   // PrototypeSet RS=PGUtils.readPrototypeSet(preprocessedPath.toString());
    ...
    SetRanges conversor = new SetRanges(dataPath, datasetPath, outputPath, getConf(), cabecera.getHeader());
    
 

    conversor.run();
    
    double[][] results = conversor.getResults();
