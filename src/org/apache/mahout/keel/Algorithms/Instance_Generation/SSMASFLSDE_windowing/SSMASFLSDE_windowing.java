...
public class SSMASFLSDE_windowing extends Metodo {
  ...
  public PrototypeSet resultingSet;
  ...
  private static final Logger log = LoggerFactory.getLogger(SSMASFLSDE_windowing.class);	
  ...
  public SSMASFLSDE_windowing (String ficheroScript) {
	super (ficheroScript);
    
  }


  public SSMASFLSDE_windowing(String ficheroScript, InstanceSet train, Context context) {
		super (ficheroScript, train, context);
		
  }
  
  public SSMASFLSDE_windowing(String ficheroScript, InstanceSet train, Context context, int sizeStrato) {
		super (ficheroScript, train, context,sizeStrato);
  }
 ...
 public void establishContext(Context context){
...
/**
   ...
   * @param actual
   */
  public double lsff(double Fi, double CRi, PrototypeSet population[], int actual, int mejor, PrototypeSet trainingDataSet){
   ...
   * @return
   */
  public PrototypeSet SFGSS(PrototypeSet population[], int actual, int mejor, double CRi, PrototypeSet trainingDataSet){
	  double a=0.1, b=1;
	  ...
	  for (int i=0; i<this.iterSFGSS; i++){ // Computation budjet
		  ...
		  fitnessFi1 = lsff(fi1, CRi, population,actual,mejor, trainingDataSet);
		  fitnessFi2 = lsff(fi2, CRi,population,actual,mejor, trainingDataSet);
  ...
  public  PrototypeSet SFHC(PrototypeSet population[], int actual, int mejor, double SFi, double CRi, PrototypeSet trainingDataSet){
	  ...
	  for (int i=0; i<this.iterSFHC; i++){ // Computation budjet
		  ...
		  fitnessFi1 = lsff(SFi-h, CRi, population,actual,mejor, trainingDataSet);
		  fitnessFi2 = lsff(SFi, CRi,  population,actual,mejor, trainingDataSet);
		  fitnessFi3 = lsff(SFi+h, CRi,  population,actual,mejor, trainingDataSet);
		   ...
		   for(int i=0; i<PopulationSize; i++){
			   ...
			   if(i==bestFitnessIndex && randj[4] < tau[2]){
				  // System.out.println("SFGSS applied");
				   //SFGSS
				   crossover[i] = SFGSS(population, i, bestFitnessIndex, CrossOverRate[i],trainingSet[i%this.stratos]);
				   
				   
			   }else if(i==bestFitnessIndex &&  tau[2] <= randj[4] && randj[4] < tau[3]){
				   //SFHC
				   //System.out.println("SFHC applied");
				   crossover[i] = SFHC(population, i, bestFitnessIndex, ScalingFactor[i], CrossOverRate[i],trainingSet[i%this.stratos]);
			   ...
			   nominalPopulation = new PrototypeSet();
		       nominalPopulation.formatear(population[i]);
		       fitness[i] = classficationAccuracy1NN(nominalPopulation,trainingSet[i%this.stratos]);
		       
		       nominalPopulation = new PrototypeSet();
		       nominalPopulation.formatear(crossover[i]);
		       
			   double trialVector = classficationAccuracy1NN(nominalPopulation,trainingSet[i%this.stratos]);
  ...
  public void ejecutar () {
	...
	log.info("SSMASFLSDE_windowing: Empezando ejecución.\n");
	log.info("SSMASFLSDE_windowing: Context "+this.context.getJobID()+"\n");
    ...
    // si es mayor que el tamaño del estrato, se considerará muy largo y por
    // tanto no se precomputa la matriz.
    // en caso contrario, estamos en el caso base.
    if (datosTrain.length > this.strataSize) {
      veryLarge = true;
    } else {
      veryLarge = false;
    }

    System.out.println("EjecutandoSSMA");
    
    // hacer estratos para SSMA.
	...
	log.info("SSMASFLSDE_windowing: Matriz de distancias computada.");

    /*Random inicialization of the population*/
    Randomize.setSeed (semilla);
    poblacion = new Cromosoma[tamPoblacion];
    for (i=0; i<tamPoblacion; i++){
      // log.info("SSMASFLSDE_windowing: Initialize individual."+i+"\n");
       poblacion[i] = new Cromosoma (kNeigh, datosTrain.length, dMatrix, datosTrain, realTrain, nominalTrain, nulosTrain, distanceEu, context);
       this.context.progress();
    }
    
	log.info("SSMASFLSDE_windowing: Initialization finish.");
	...
	log.info("SSMASFLSDE_windowing: Evaluation finish.");
   ...
   //	log.info("SSMASFLSDE_windowing: Comenzando bucle.");
      ...
      /*Evaluation of offsprings*/
  	 // log.info("SSMASFLSDE_windowing: Hijos generados.");
	  this.context.progress();
	  ...
	  this.context.progress();
	 // log.info("SSMASFLSDE_windowing: eval "+ev+"\n");
...
/*      System.out.println(ev + " - (" + umbralOpt + ")" +
	  ...
	  log.info("SSMASFLSDE_windowing: fin bucle.\n");
   ...
   // System.out.println("**************FUERA");
    ...
    this.context.progress();
    this.resultingSet = reduceSet(training); // LLAMO al SADE

    System.out.println("Time elapsed:" + (double)(System.currentTimeMillis()-tiempo)/1000.0 + "s");
