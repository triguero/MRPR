 ...
 */
public abstract class PrototypeGenerationAlgorithmHadoop<T extends PrototypeGenerator>
{
    ...
    protected void configureJob(Job job) throws IOException {
	    ...
	    job.setJarByClass(PrototypeGenerationAlgorithmHadoop.class);
	    job.setJobName("Prototype Generation");
	   ...
	   // FileOutputFormat.setOutputPath(job, getOutputPath(conf));
	    ...
	    job.setMapperClass(PrototypeGenerationMapper.class);
	    job.setReducerClass(PrototypeGenerationReducer.class);
    ...
    /**
     * Gets the file names from one line oforg.apache.mahout.keel.text.
    ...
    /**
     * Read theorg.apache.mahout.keel.parameters file. Load all parameters of the algorithm.
 ...
 //   protected abstract T buildNewPrototypeGenerator(PrototypeSet train,  PrototypeSet test, Parameters params);
    
    
    /**
     * Assertorg.apache.mahout.keel.style arguments
    ...
    /**
     * Execute the algorithm given, and return the classification done in TRAINING AND TEST!
     * It was NECESSARY for StratPGFW
     * @param args Arguments given by console.
    ...
    public void execute2(String[] args)
    {
    	try{
    	   ...
    	   // training.print();
    	    PrototypeGenerator generator = buildNewPrototypeGenerator(training, test, new Parameters(parameters));
    	    Pair<PrototypeSet, PrototypeSet> resultingSet  = generator.execute2();
            
    	    int trainingRealClass[][];
            int trainingPrediction[][];
            int testRealClass[][];
            int testPrediction[][];
            
            trainingRealClass = new int[training.size()][1];
            trainingPrediction = new int[training.size()][1];	
            testRealClass = new int[test.size()][1];
    	    testPrediction = new int[test.size()][1];	
    	    
            int nClases = training.getPosibleValuesOfOutput().size();
                
            for (int i=0; i< training.size(); i++) {
            	trainingRealClass[i][0] = (int) training.get(i).getOutput(0);
            	trainingPrediction[i][0] = (int) resultingSet.first().get(i).getOutput(0);
            }
    	    ...
    	    writeOutput(outputFilesPath.get(0), trainingRealClass, trainingPrediction, entradas, salida, relation);
    ...
    /**
     * Execute the algorithm given para no balanceados
     * @param args Arguments given by console.
     */
    public void executeNB(String[] args)
    {
    	try{
    		long tiempo = System.currentTimeMillis();
            Parameters.assertBasicArgs(args);
    	   ...
    	   // training.print();
    	    PrototypeGenerator generator = buildNewPrototypeGenerator(training, new Parameters(parameters));
    	    PrototypeSet resultingSet = generator.executeNB();
            ...
            int pre[] = this.classify(PrototypeGenerator.classifier, resultingSet, training);
           //Working on training
            for (int i=0; i<training.size(); i++) {
                 trainRealClass[i][0] = (int) training.get(i).getOutput(0);
                 trainPrediction[i][0] = pre[i];
            }
	        ...
	        long tiempo_test = System.currentTimeMillis();
	        
	        pre = this.classify(PrototypeGenerator.classifier, resultingSet, test);
	        
            for (int i=0; i<realClass.length; i++) {
		       realClass[i][0] = (int) test.get(i).getOutput(0);
		       prediction[i][0]= pre[i];
            }
            
            System.out.println("Test time: " + (double)(System.currentTimeMillis()-tiempo_test)/1000.0 + "");
    	  ...
    	  // System.out.println("Resulting size= "+ resultingSet);
    	   ...
    	   // resultingSet.save(outputFilesPath.get(TRAINING));
            ...
            System.out.println("Time elapsed: " + (double)(System.currentTimeMillis()-tiempo)/1000.0 + "");
    ...
    /**
     * Execute the algorithm given with weights features.
     * @param args Arguments given by console.
     */
    public void executeFeatures(String[] args)
    {
    	try{
    		long tiempo = System.currentTimeMillis();
            Parameters.assertBasicArgs(args);
    	   ...
    	   // training.print();
    	    PrototypeGenerator generator = buildNewPrototypeGenerator(training, new Parameters(parameters));
    	    Pair<PrototypeSet, double[]> result = generator.executeFeatures();
            
    	    
    	    PrototypeSet resultingSet = result.first();
    	    double[] weights = result.getSecond().clone();
           ...
           //Working on training
            for (int i=0; i<training.size(); i++) {
                 trainRealClass[i][0] = (int) training.get(i).getOutput(0);
                 trainPrediction[i][0] = evaluateWeighting(training.get(i).getInputs(),resultingSet.prototypeSetTodouble(), nClases, resultingSet.getClases(), 1, weights);
            }
            ...
            for (int i=0; i<realClass.length; i++) {
		       realClass[i][0] = (int) test.get(i).getOutput(0);
		       prediction[i][0]= evaluateWeighting(test.get(i).getInputs(),resultingSet.prototypeSetTodouble(), nClases, resultingSet.getClases(), 1, weights);
            }
	...
	protected static double distanceWeighting(double instance1[],double instance2[], double Weights[]){
	  ...
	  /** 
	 * Evaluates a instance to predict its class. WITH WEIGHTING SCHEME
	 * 
	 * @param example Instance evaluated 
	 * @return Class predicted
	 * 
	 */
	public static int evaluateWeighting (double example[], double trainData[][],int nClasses,int trainOutput[],int k, double Weights[]) {
		...
		for (int i=0; i<trainData.length; i++) {
		    ...
		    dist = distanceWeighting(trainData[i],example, Weights);
	 ...
	 public int[] classify(String classifier, PrototypeSet training, PrototypeSet test)
	  {
		...
		if(classifier.equals("NN")){
		  ...
		  if(training.size()>PrototypeGenerator.kNearest){
		      predicho = KNN.classify2(training, test, PrototypeGenerator.kNearest).clone();
		  }else{
			...
			try {
				 ...
				 // training.save("train1.dat");
				  ...
				  SMO = new HandlerSMO(training.toInstanceSet(), test.toInstanceSet(),2,String.valueOf(78976543));
				
				  //SMO.generateFiles();
				  
			      predicho = SMO.getPredictions(0);    
