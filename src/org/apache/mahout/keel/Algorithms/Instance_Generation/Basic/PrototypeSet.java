...
import org.apache.mahout.keel.Algorithms.Instance_Selection.SSMA.Cromosoma;
 ...
 */
public class PrototypeSet extends ArrayList<Prototype> implements Comparable, Writable
{
    /** Associated instance set to the prototype set. */
    protected static InstanceSet associatedInstanceSet = null;
    ...
    // hadoop conversion.
    public PrototypeSet(Data s){
    	
        int num = s.size();
        double labels[][]= new double[num][1];
        // convert string to double
 
        for(int i=0; i<num; ++i)
        {
        	add(new Prototype(s.get(i).get()));
        }
    ...
    // hadoop conversion para Jesús.
    public PrototypeSet(org.apache.mahout.classifier.KnnMR.data.Data s){
    ...
    public PrototypeSet(Data s, Context context){
    ...
    // Con el tiempo solo quedará esta, todo a la carpeta BASIC
    
    public PrototypeSet(org.apache.mahout.classifier.basic.data.Data s, Context context){
  ...
  public InstanceSet toInstanceSet(){
        ...
        s.setHeader(InstanceSet.getHeader());
        s.setAttHeader(InstanceSet.getAttHeader());
//    	System.err.println(s);
    	...
    	for(int i=0; i< this.size(); ++i)
        {
            ...
            */
   		    Instance anadir = new  Instance(this.get(i).getInputs(), InstanceSet.getAttributeDefinitions());
           ...
           Prototype q = this.get(i).denormalize(); //TOKADO PARA NO NORMALIZAR
             ...
             for(int j=0; j<this.get(i).numberOfInputs(); ++j)
               ...
               if(nominal_output){
                 ...
                 //System.out.println(q.getOutputAsNominal(0));
  ...
  public InstanceSet toInstanceSetNODENORMALIZE(){
...
//  	System.err.println(s);
     ...
     * @param title Title of the data set (required for allorg.apache.mahout.keel.data files)
    ...
    public String asKeelDataFileString(String title)
    {
        ...
        System.out.println("n_attributes: "+n_attributes);
        for(Prototype p: this)
        {
   ...
   public static PrototypeSet selecRandomSet(PrototypeSet original, int numberOfPrototypesSelected, boolean usePriorProb) {
       ...
       else
       {
		...
		//	System.out.println("size = " + size);
    		int features=out.readInt();
		//	System.out.println("features = " + features);

    		
	    	for(int i=0; i< size;i++){
	    		Prototype aux = new Prototype(features,1);
	    		aux.readFields(out);
	    		this.add(aux); // añadir función write a la clase Prototype.
	    	}
	    	
	    //	System.out.println("Leyendo conjunto.");
	    //	this.print();
		}
		...
		public void write(DataOutput out) throws IOException {
			// TODO Apéndice de método generado automáticamente
		//	System.out.println("Primero escribo");

			//number of instances
			out.writeInt(this.size());
			
			// number of features 
			out.writeInt(this.get(0).numberOfInputs());
			
	    	for(int i=0; i< this.size();i++){
	    		this.get(i).write(out); // añadir función write a la clase Prototype.
	    	}
		...
		public static PrototypeSet read(DataInput dataInput)throws IOException {
			PrototypeSet fc = new PrototypeSet();
		    fc.readFields(dataInput);
		    return fc;
		}
   
		  /**
		   * Load the RS set bases from a single file 
		   * @throws java.io.IOException
		   */
		  public static PrototypeSet load(Configuration conf, Path rsPath) throws IOException {
		    FileSystem fs = rsPath.getFileSystem(conf);
		    Path[] files;
		    if (fs.getFileStatus(rsPath).isDir()) {
		      files = PGUtils.listOutputFiles(fs, rsPath);
		    } else {
		      files = new Path[]{rsPath};
		    }

		    PrototypeSet fc = null;
		    for (Path path : files) {
		   ...
		   * @throws java.io.IOException
		   */
		  public static PrototypeSet load(Configuration conf, Path rsPath, String cabecera) throws IOException {
			  
		    PGUtils.readHeader(cabecera);
