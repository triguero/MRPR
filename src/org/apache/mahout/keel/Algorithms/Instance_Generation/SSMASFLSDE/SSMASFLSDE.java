...
package org.apache.mahout.keel.Algorithms.Instance_Generation.SSMASFLSDE;
...
public class SSMASFLSDE extends Metodo {
  ...
  private static final Logger log = LoggerFactory.getLogger(SSMASFLSDE.class);	
  ...
  public SSMASFLSDE(String ficheroScript, InstanceSet train, Context context) {
...
/**
  ...
  public double lsff(double Fi, double CRi, PrototypeSet population[], int actual, int mejor){
     ...
     //  FitnessFi =  classficationAccuracy1NN(nominalPopulation,trainingDataSet);
	   FitnessFi =  classficationAccuracy1NN(crossover,trainingDataSet);
  ...
  public PrototypeSet reduceSet(PrototypeSet initial)
  {
	 ...
	 // population[0].print();
	  ...
	  //System.out.println("PROBANDOOO "+this.numberOfClass);
	  // Aseguro que al menos hay un representante de cada clase. Si SSMA FALLA.
	   ...
	   for(int i=0; i< population[0].size(); i++){
		  for(int j=0; j< this.numberOfClass; j++){
			  if(population[0].getFromClass(j).size() ==0 && clases[j].size()!=0){
				  ...
				  System.out.println("Adding instance of class: "+j);
			  }
		  }
	  }
	 ...
	 double bestFitness=fitness[0];
	  ...
	  for(int i=1; i< PopulationSize;i++){
	   ...
	   for(int iter=0; iter< MaxIter; iter++){ // Main loop
		   ...
		   for(int i=0; i<PopulationSize; i++){
		     ...
		     //  fitness[i] = classficationAccuracy1NN(nominalPopulation,trainingDataSet);
		      ...
		      // nominalPopulation.formatear(crossover[i]);
		       ...
		       double trialVector = classficationAccuracy1NN(crossover[i],trainingDataSet);
           ...
           System.err.println("\n% de acierto en training " + classficationAccuracy1NN(population[bestFitnessIndex],trainingDataSet) );
  ...
  public void ejecutar () {
	...
	log.info("SSMASFLSDE: Empezando ejecución.\n");
	log.info("SSMASFLSDE: Context "+this.context.getJobID()+"\n");
	...
	log.info("SSMASFLSDE: Matriz de distancias computada.");
    ...
    for (i=0; i<tamPoblacion; i++){
      // log.info("SSMASFLSDE: Initialize individual."+i+"\n");
	...
	log.info("SSMASFLSDE: Initialization finish.");
	...
	log.info("SSMASFLSDE: Evaluation finish.");
   ...
   //	log.info("SSMASFLSDE: Comenzando bucle.");
      ...
      /*Evaluation of offsprings*/
  	 // log.info("SSMASFLSDE: Hijos generados.");
	  this.context.progress();
