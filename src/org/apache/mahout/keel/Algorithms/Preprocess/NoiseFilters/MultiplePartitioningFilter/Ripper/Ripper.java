 ...
 */
  ...
  /**
   * It reads the data from the INSTANCESETS and parse all the parameters
   * from the parameters array.
   * @param parameters parseParameters It contains the input files, output files and parameters
   */
  public Ripper(parseParameters parameters, InstanceSet trainI, InstanceSet testI, int clases) {
    ...
    try {
      //System.out.println("\nReading the training set: " + parameters.getTrainingInputFile());
      train.readClassificationSetFromInstanceSet(trainI, clases);
      //System.out.println("\nReading the validation set: " + parameters.getValidationInputFile());
      val.readClassificationSetFromInstanceSet(trainI, clases);
      //System.out.println("\nReading the test set: " + parameters.getTestInputFile());
      test.readClassificationSetFromInstanceSet(testI, clases);
    } catch (IOException e) {
      System.err.println("There was a problem while reading the input data-sets: " + e);
      somethingWrong = true;
    }
  ...
  public Ruleset[] runModel() {
    if (somethingWrong) { //We do not execute the program
      ...
      doRulesOutput2("salida.txt",rulesets);
      cubrirReglas(rulesets);
      return rulesets;
    }
  }
   ...
   */
  private void cubrirReglas(Ruleset[] rulesets) {
     int i;
    for (i=0;i<rulesets.length-1;i++){
      ...
      for(int j=0;j<rulesets[i].size();j++){
        ...
        current.confidence = (double)accuracy / (double)covered;
      }
    }
    Mask class_filter=new Mask(train.size());
    train.filterByClass(class_filter,rulesets[i].getType());
    Rule current=rulesets[i].getRule(0);
    int accuracy=current.apply(train,class_filter);
    int covered = 0;//train.numEjemplos(train.claseNumerica(rulesets[i].getType()));
    rulesets[i].getRule(0).confidence = (double)accuracy / (double)covered;
  }
