 ...
 */ 
   ...
   * @throws IOException If there ocurs any problem with the reading of the data-set
   */
  public void readClassificationSetFromInstanceSet(InstanceSet train, int clases) throws IOException {
    try {
      // Load in memory a dataset that contains a classification problem
      //IS.readSet(datasetFile, train);
      ...
      for (int i = 0; i < nData; i++) {
        ...
        if (noOutputs) {
          ...
          //outputInteger[i] = (int) IS.getOutputNumericValue(i, 0);
          output[i] = IS.getOutputNominalValue(i, 0);
          //System.out.println(outputInteger[i]);
          //System.out.println(Attributes.getOutputAttribute(0).convertNominalValue(output[i]));
          outputInteger[i] = Attributes.getOutputAttribute(0).convertNominalValue(output[i]);
