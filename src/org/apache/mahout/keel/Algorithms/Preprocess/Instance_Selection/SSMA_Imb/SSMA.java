...
public class SSMA extends Metodo {
	...
	public SSMA (String ficheroScript, InstanceSet train) {
	...
	public void ejecutar (Context context) {
		...
		System.out.println("I am executing SSMA_Imb");
        ...
        // fix after modification for windowing
        ArrayList<Integer> elsToEvaluatePos = new ArrayList<Integer>();
        for(int el = 0; el < posIndices.length; el++){
        	elsToEvaluatePos.add(el);
        }
		ArrayList<Integer>  elsToEvaluateNeg = new ArrayList<Integer>();
		for(int el = 0; el < negIndices.length; el++){
        	elsToEvaluateNeg.add(el);
        }
        // end fix
